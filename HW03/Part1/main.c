...
int main (int argc, char **argv) {
  ...
  if(rank==1) {
    ...
    printf("]\n");
    MPI_Send(message, Nmessages, MPI_INT, 0, tagm, MPI_COMM_WORLD);
    MPI_Send(a, Nmessages, MPI_INT, 0, taga, MPI_COMM_WORLD);
  }
  ...
  if(rank ==0) {
    MPI_Status stat;
    MPI_Recv(message, Nmessages, MPI_INT, 1, tagm, MPI_COMM_WORLD, &stat);
    MPI_Recv(a, Nmessages, MPI_INT, 1, taga, MPI_COMM_WORLD, &stat);
